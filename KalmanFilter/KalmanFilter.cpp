                           ...
                           MatrixXd x0input, unsigned int maxSimulationSamples)
{
    ...
    //this matrix is used to store the a posteriori estimates column wise
    estimateAposteriori.resize(n, maxSimulationSamples); //rows = n, columns = MaxSimulationSamples
    estimateAposteriori.setZero(); //set all of the Matrix entries to zero to pre-allocate the space for this Matrix.
    estimateAposteriori.col(0) = x0; //starting from x0

    //this matrix is used to store a priori estimates column wise
    estimateApriori.resize(n, maxSimulationSamples);
    estimateApriori.setZero();

    //this matrix is used to store the a posteriori covariance matrices
    covarianceAposteriori.resize(n, n*maxSimulationSamples);
    covarianceAposteriori.setZero();
    covarianceAposteriori(all, seq(0,n-1)) = P0;

    // this matrix is used to store the a priori covariance matrices
    covarianceApriori.resize(n, n*maxSimulationSamples);
    covarianceApriori.setZero();

    //this matrix is used to store the Kalman gain matrices 
    gainMatrices.resize(n, r*maxSimulationSamples);
    gainMatrices.setZero();

    //this matrix is used to store the errors 
...
void KalmanFilter::predictEstimate(MatrixXd externalInput)
{
    ...
    estimateApriori.col(k) = A * estimatesAposteriori.col(k) + B * externalInput;
    covarianceApriori(all, seq(k*n, (k+1)*n - 1)) = A8 covarianceAposteriori(all,seq(k*n, (k+1)*n -1)) * (A.transpose()) + Q;
...
void KalmanFilter::updateEstimate(MatrixXd measurement)
{
    ...
    MatrixXd Sk;
    Sk.resize(r,r);
    Sk = R + C*covarianceApriori(all, seq(k*n, (k+1)*n - 1)) * (C.transpose());
    Sk = Sk.inverse();
    ...
    //compute the error
    errors.col(k-1) = measurement - C * estimateApriori.col(k-1);

    //compute the a posterior estimate, remember that for k =0, the corresponding column is x0 - initial estimate
    estimatesAposteriori.col(k) = estimateApriori.col(k-1) + gainMatrices(all, seq((k-1)*r, k*r-1)) * errors.col(k-1);
    ...
    covarianceAposteriori(all, seq(k*n, (k+1)*n - 1)) = IminusKC * covarianceApriori(all, seq((k-1)*n, k*n-1)) * IminusKC.transpose() 
							...
							string gainMatricesFile, string errorsFile) const
{
	...
	if (file2.is_open())
	{
		file2 << estimateApriori.format(CSVFormat);
		file2.close();
	}
